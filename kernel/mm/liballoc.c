/*
 * Durand Amazing Super Duper Memory functions — the allocator ALGORITHM
 * only. Unmodified upstream logic; split out of kheap.c so userspace can
 * share it (usr/lib/malloc.c) instead of carrying a second, separately
 * debugged allocator.
 *
 * Everything OS-specific lives behind <liballoc_config.h>, which each
 * side supplies on its own include path: the kernel's names these kmalloc
 * / kfree and backs them with vmm_alloc_pages (kernel/mm/kheap.c), the
 * userspace one names them malloc / free and backs them with
 * SYS_ANON_ALLOC. Same code, two configurations.
 */
